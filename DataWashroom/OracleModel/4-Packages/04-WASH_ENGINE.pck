...
FUNCTION GET_TABLE_NAME_IN_TRIGGER
(
	inCall_Stack	VARCHAR2
)	RETURN			VARCHAR2
IS
	...
	tTab_Schema		VARCHAR2(30);
	tTab_Name		VARCHAR2(30);
BEGIN
	SELECT
		TABLE_OWNER,
		TABLE_NAME
...
BEGIN
	...
	FROM
		...
		VPI.WASH_ISTR		I
	WHERE
			T.ISTR_TYPE		= I.ISTR_TYPE
		AND	I.ISTR_ID		= inIstr_ID;

	IF GET_TABLE_NAME_IN_TRIGGER(inCall_Stack) != tDerived_Table THEN
		RAISE_APPLICATION_ERROR(-20011, UTL_LMS.FORMAT_MESSAGE('The ISTR_TYPE "%s" (in WASH_ISTR) expects the derived table "%s"', tIstr_Type, tDerived_Table));
	END IF;
END CHECK_ISTR_TYPE;
...
BEGIN
	...
	LOOP
		PLAN_DELETE(L.CYCLE_ID, L.ISTR_ORDER, L.ISTR_ID, L.DST_TABLE, L.DST_FILTER, L.TYPE_BRIEF, L.DESCRIPTION_);
	END LOOP;
	...
	(
		...
		WHERE
				T.ISTR_TYPE	= B.ISTR_TYPE
			AND	C.ISTR_ID	= B.ISTR_ID
			AND	B.CYCLE_ID	= inCycle_ID
	)
	LOOP
		PLAN_COPY(L.CYCLE_ID, L.ISTR_ORDER, L.ISTR_ID, L.SRC_VIEW, L.SRC_FILTER, L.DST_TABLE, L.TYPE_BRIEF, L.DESCRIPTION_);
	END LOOP;
	...
	(
		...
		WHERE
				T.ISTR_TYPE	= B.ISTR_TYPE
			AND	M.ISTR_ID	= B.ISTR_ID
			AND	B.CYCLE_ID	= inCycle_ID
	)
	LOOP
		PLAN_MERGE(L.CYCLE_ID, L.ISTR_ORDER, L.ISTR_ID, L.SRC_VIEW, L.SRC_FILTER, L.DST_TABLE, L.JOIN_COLUMNS, L.UPDATE_COLUMNS, L.INSERT_COLUMNS, L.TYPE_BRIEF, L.DESCRIPTION_);
	END LOOP;
	...
	(
		...
		WHERE
				T.ISTR_TYPE	= B.ISTR_TYPE
			AND	U.ISTR_ID	= B.ISTR_ID
			AND	B.CYCLE_ID	= inCycle_ID
	)
	LOOP
		PLAN_CHECK_UNIQUE_KEY(L.CYCLE_ID, L.ISTR_ORDER, L.ISTR_ID, L.DST_TABLE, L.DST_FILTER, L.KEY_COLUMNS, L.SET_EXPR_IF_UNIQUE, L.SET_EXPR_IF_DUPLICATE, L.TYPE_BRIEF, L.DESCRIPTION_);
	END LOOP;
	...
	LOOP
		PLAN_SORT_DUP_KEY(L.CYCLE_ID, L.ISTR_ORDER, L.ISTR_ID, L.DST_TABLE, L.DST_FILTER, L.KEY_COLUMNS, L.ORDER_BY, L.RN_COLUMN, L.TYPE_BRIEF, L.DESCRIPTION_);
	END LOOP;
	...
	LOOP
		PLAN_MAKE_UP_MISSING_KEY(L.CYCLE_ID, L.ISTR_ORDER, L.ISTR_ID, L.SRC_VIEW, L.SRC_FILTER, L.SRC_KEY_COLUMNS, L.DST_TABLE, L.DST_KEY_COLUMNS, L.DST_VAL_COLUMNS, L.SRC_VALUES, L.TYPE_BRIEF, L.DESCRIPTION_);
	END LOOP;
	...
	(
		...
		WHERE
				T.ISTR_TYPE	= B.ISTR_TYPE
			AND	N.ISTR_ID	= B.ISTR_ID
			AND	B.CYCLE_ID	= inCycle_ID
	)
	LOOP
		PLAN_MAKE_UP_NA_KEY(L.CYCLE_ID, L.ISTR_ORDER, L.ISTR_ID, L.DST_TABLE, L.DST_KEY_COLUMNS, L.DST_VAL_COLUMNS, L.MAKE_UP_KEYS, L.MAKE_UP_VALUES, L.TYPE_BRIEF, L.DESCRIPTION_);
	END LOOP;
